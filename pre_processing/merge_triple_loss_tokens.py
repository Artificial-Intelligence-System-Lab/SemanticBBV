...
def load_file(file_path, result_queue, error_queue, min_seq_len, max_seq_len):
    """
    Load a single file, filter its sequences by length
    ...
    try:
        with open(file_path, 'rb') as f:
            funcs_in_file = pickle.load(f) # This is a dict of different optimized levels of the same func
            filtered_sequences = {}
            for key, value in funcs_in_file.items():
                function_length = max([len(level['asm']) for level in value])
                if min_seq_len < function_length < max_seq_len and len(value) == 5:
                    filtered_sequences[key] = value
            result_queue.put((file_path, filtered_sequences))
    except Exception as e:
        error_queue.put((file_path, f"Error processing file {file_path}: {e}"))

def merge_token_files(directory_path: str, output_file: str, num_threads: int = None, min_length: int = None, max_length: int = None):
    """
    ...
    for root, _, files in os.walk(directory_path):
        for file in files:
            if file == "saved_func_tokens_cached.pkl":
                file_path = os.path.join(root, file)
                file_paths.append(file_path)
    
    total_input_files = len(file_paths)
    if total_input_files == 0:
        print(f"No 'saved_func_tokens_cached.pkl' files found in directory {directory_path}.")
        return False
    ...
    for i in range(0, total_input_files, files_per_chunk):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            # Submit all tasks
            futures = [executor.submit(load_file, file_path, result_queue, error_queue, min_length, max_length) 
        ...
        while not result_queue.empty():
            ...
            all_tokens_in_chunk.update(tokens)
            processed_files_in_chunk += 1
        
        if not all_tokens_in_chunk:
            print(f"Chunk {chunk_index} has no data successfully loaded, skipping save.")
            continue

        # Build current chunk output filename
        current_output_file = f"{output_file_base}_{chunk_index}{output_file_ext}"
        
        # Save current chunk merged data
        try:
            print(f"Saving {len(all_tokens_in_chunk)} merged training data for chunk {chunk_index} to {current_output_file}")
    ...
    if processed_chunks_count > 0:
        print(f"\nAll chunks processed! Generated {processed_chunks_count} merged files in total.")
        print(f"Total {total_tokens_saved_across_chunks} token data saved.")
        return True
    else:
...
def main():
    ...
    parser.add_argument('-j', '--jobs', type=int, default=None, 
                        help='Number of threads for parallel processing, defaults to 2x CPU cores')
    parser.add_argument('--min-length', type=int, default=0,
                        help='Minimum sequence length for merging (inclusive)')
    parser.add_argument('--max-length', type=int, default=512,
                        help='Maximum sequence length for merging (inclusive)')
    
    args = parser.parse_args()
    
    # Validate sampling ratio parameters
    
    # Set garbage collection threshold for more aggressive memory reclamation
    gc.set_threshold(100, 5, 5)  # Default values are (700, 10, 10)
    
    # Merge files
    start_time = time.time()
    merge_token_files(args.directory, args.output, args.jobs, args.min_length, args.max_length)
