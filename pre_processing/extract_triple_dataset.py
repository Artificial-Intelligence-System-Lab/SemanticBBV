...
def merge_pickle_files(directory_path, output_file, file_pattern="*.pkl"):
    ...
    Args:
        ...
        file_pattern (str): Filename matching pattern, defaults to "train_dataset_*.pkl"
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                if isinstance(data, list):
                    ...
                    for idx in indices:
                        positives = random.sample(data[idx], 2)
                        anchor = positives[0]
                        positive = positives[1]
                        # Before optimization: negative = random.choice(data[random.choice([i for i in range(len(data)) if i != idx])])
                        # After optimization: directly use random selection then verify
                        neg_idx = idx
                        while neg_idx == idx:
                            neg_idx = random.randrange(len(data))
                        negative = random.choice(data[neg_idx])
                        all_data.append((anchor, positive, negative))
                        total_sequences += 1
                else:
...
def main():
    ...
    parser.add_argument('-p', '--pattern', default='*.pkl',
                        help='Filename matching pattern, defaults to "*.pkl"')
