...
def load_model(checkpoint_path, vocabs_size, device="cuda", n_embd=768, n_layer=6, head_size=64, use_bf16=True):
    ...
    class RWKV7Model(nn.Module):
        def __init__(self, args, vocab_size):
            ...
            self.individual_dims = 128
            concatenated_n_embd = self.individual_dims * self.num_embedding_types

            self.asm_embd = nn.Embedding(vocab_size["asm"], self.individual_dims)
            self.mne_embd = nn.Embedding(vocab_size["mne"], self.individual_dims)
            self.type_embd = nn.Embedding(vocab_size["type"], self.individual_dims)
            self.reg_embd = nn.Embedding(vocab_size["reg"], self.individual_dims)
            self.rw_embd = nn.Embedding(vocab_size["rw"], self.individual_dims)
            self.eflag_embd = nn.Embedding(vocab_size["eflag"], self.individual_dims)

            self.embedding_projection_layer = nn.Linear(concatenated_n_embd, args.n_embd)
            # (Optional) Can add activation or layer normalization after projection layer
            self.projection_layernorm = nn.LayerNorm(args.n_embd)
        ...
        def forward(self, token_inputs, layer_idx=None):
            ...
            # Pass concatenated embedding through projection layer
            projected_emb = self.embedding_projection_layer(combined_emb_concatenated)
            # (Optional) Apply activation or layer normalization
            projected_emb = self.projection_layernorm(projected_emb)

            # Pass combined embedding to model
            if layer_idx is not None:
                token_hidden = self.model(projected_emb, layer_idx=layer_idx)
            else:
    ...
    model = RWKV_x070(args, checkpoint_path)
...
def get_instruction_encodings(model, asm_instructions, encoding=None, layer_idx=None, device="cuda", max_samples=10000):
    """
    Get encoding for each assembly instruction, since model does not support batch, changed to process one by one
    """
    encodings = []
    error_count = 0
    ...
    print(f"Starting to process instructions one by one...")
    # Changed to process instructions one by one
    for i, asm in enumerate(asm_instructions):
        try:
            tokens = encoding[asm]
            
            with torch.no_grad():
                # Convert to single sample tensor
                single_tokens = {}
                for key, value in tokens.items():
                    single_tokens[key] = value
                print(single_tokens)
                
                # Call model for inference - use forward method instead of direct call
                hidden, state = model.forward(single_tokens, state=None, full_output=True)

                # Extract KV cache from all layers and perform weighted average
                def get_weighted_kv_cache(state, num_layers=6):
                    """Extract KV cache from all layers and perform weighted average, shallow layers have high weight, deep layers low weight
                    
                    Args:
                        state: model state list containing states of all layers
                        num_layers: number of model layers, default 6
                        
                    Returns:
                        Weighted average KV cache tensor (flattened)
                    """
                    # Collect KV cache from all layers
                    kv_caches = []
                    for i in range(num_layers):
                        # Second tensor in each layer is KV cache, index i*3+1
                        kv_cache = state[i*3+1]
                        kv_caches.append(kv_cache)
                    
                    # Calculate weights, shallow layers have high weight, deep layers have low weight
                    weights = torch.tensor(
                        [1.0 - (i / num_layers) * 0.8 for i in range(num_layers)],  # Weights decrease from 1.0 to 0.2
                    ...
                    # Flatten each KV cache
                    flattened_caches = [cache.flatten() for cache in kv_caches]
                    
                    # Weighted sum
                    weighted_sum = torch.zeros_like(flattened_caches[0])
                    for i, cache in enumerate(flattened_caches):
                        weighted_sum += cache * weights[i]
                    
                    return weighted_sum
                
                # Use weighted KV cache to replace original only using 3rd layer KV cache
                flattened_state = get_weighted_kv_cache(state)
                
                # Initialize random mapping matrix (only on first time)
                if random_matrix is None:
                    torch.manual_seed(42)
                    dtype = state[5*3+1].dtype
                    random_matrix = torch.randn(flattened_state.shape[0], 20, dtype=dtype).to(device)
                    print(f"Initialized random mapping matrix, shape: {random_matrix.shape}, data type: {dtype}")
                
                print(flattened_state.shape)
                
                # Use random mapping matrix to map hidden states to length 20 vector
                encoding_vector = torch.matmul(flattened_state, random_matrix).float().cpu().numpy()
                
                result = {
                    'instruction': asm,
                    'encoding': encoding_vector.tolist()
                }
            ...
            import traceback
            print(f"Error processing instruction '{asm}': {str(e)}")
            traceback.print_exc()
            error_count += 1
            pbar.update(1)
            continue
...
def get_instruction_encodings_batch(model, asm_instructions, batch_size=128, encoding=None, layer_idx=None, device="cuda", max_samples=10000):
    """
    ...
    for i in range(0, len(all_tokens), batch_size):
        ...
        with torch.no_grad():
            ...
            if not hasattr(get_instruction_encodings, 'random_matrix'):
                ...
                # Ensure random matrix uses same data type as model
                dtype = hidden.dtype
                get_instruction_encodings.random_matrix = torch.randn(768, 20, dtype=dtype).to(device)
                print(f"Initialized random mapping matrix, shape: {get_instruction_encodings.random_matrix.shape}, data type: {dtype}")
            
            # Get hidden states of first non-padded position for each sequence
            mask = (batch_tokens["asm"] != PAD_ID)
            
            # Get hidden states of first non-padded position for each sample
            first_hidden_states = []
            for i in range(hidden.size(0)):
                # Get index of first non-padded position in current sequence (always 0, padding won't be at sequence start)
                first_idx = 0
                # Get hidden state at first position
                first_hidden_states.append(hidden[i, first_idx])
            
            # Convert list to tensor
            first_hidden = torch.stack(first_hidden_states)
            
            # Use random mapping matrix to map hidden states to length 20 vector
            # Ensure data types match
            encoding = torch.matmul(first_hidden, get_instruction_encodings.random_matrix).float().cpu().numpy()
...
def visualize_tsne(encodings, output_file="tsne_visualization.pdf", categories_file=None, max_k=30):
    ...
    with open(cluster_samples_file, 'w') as f:
        ...
        for label in sorted(clusters.keys()):
            ...
            # Output all samples, not just first 10
